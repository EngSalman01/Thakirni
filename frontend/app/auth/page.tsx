...
function AuthForm() {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (phoneNumber) {
      ...
      if (newUser) {
        await supabase.from("profiles").update({ phone_number: phoneNumber }).eq("id", newUser.id);
      }
    }
    setSignUpDone(true);
    setIsLoading(false);
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        className="w-full flex items-center justify-center gap-3 py-2.5 rounded-xl border border-border bg-background hover:bg-muted/50 transition-colors font-medium text-sm disabled:opacity-60"
      >
        {isGoogleLoading
        ...
        <span dir="ltr">Continue with Google</span>
      </motion.button>
    </motion.div>
  );
}
