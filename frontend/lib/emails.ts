      ...
      <table width="100%" style="max-width:560px;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
        ...
        <tr>
          <td style="background:#f8fafc;padding:24px 40px;text-align:center;border-top:1px solid #f1f5f9;">
            ${footer ?? `
            <p style="margin:0 0 4px;font-size:12px;color:#94a3b8;">إذا لم تقم بإنشاء هذا الحساب، يمكنك تجاهل هذا البريد بأمان.</p>
            <p style="margin:0 0 16px;font-size:12px;color:#94a3b8;">If you didn't create this account, you can safely ignore this email.</p>
            `}
...
function button(arText: string, enText: string, href: string): string {
  return `<table width="100%" cellpadding="0" cellspacing="0">
    <tr><td align="center" style="padding:8px 0 32px;">
      <a href="${href}" style="display:inline-block;background:linear-gradient(135deg,#2552ca,#ad1d7f);color:#ffffff;text-decoration:none;font-size:16px;font-weight:700;padding:16px 40px;border-radius:100px;letter-spacing:0.3px;">
        ${arText} · ${enText}
      </a>
    </td></tr>
  </table>`
...
function divider(): string {
  return `<hr style="border:none;border-top:1px solid #f1f5f9;margin:0 0 28px;"/>`
}
...
export async function sendWelcome(opts: { to: string; name: string }) {
  ...
  const html = layout(`
    ...
    <p style="margin:0 0 24px;font-size:15px;color:#64748b;line-height:1.7;text-align:right;">
      هلا <bdi>${name}</bdi>! يسعدنا انضمامك. ذكّرني هو مساعدك الذكي الذي يتذكر كل شيء، ينظم وقتك، ويتكلم معاك بالعربي.
    </p>
    ...
    <p style="margin:0;font-size:14px;color:#64748b;line-height:1.6;text-align:left;direction:ltr;">
      Hey <bdi>${name}</bdi>! We're glad you're here. Thakirni is your AI assistant that remembers everything, organizes your schedule, and talks to you in Arabic or English.
    </p>
  `)
...
export async function sendSubscriptionConfirmed(opts: {
  ...
  const html = layout(`
    ...
    <p style="margin:0 0 24px;font-size:15px;color:#64748b;line-height:1.7;text-align:right;">
      هلا <bdi>${name}</bdi>، تم تفعيل اشتراك <strong>${planNameAr}</strong>. تقدر الآن تستفيد من جميع المميزات.
    </p>
    ...
    <p style="margin:0;font-size:14px;color:#64748b;line-height:1.6;text-align:left;direction:ltr;">
      Hi <bdi>${name}</bdi>, your <strong>${planName}</strong> subscription is now active. Enjoy all the premium features!
    </p>
  `, `<p style="margin:0 0 16px;font-size:12px;color:#94a3b8;">هذا البريد تأكيد لعملية الدفع · This email confirms your payment.</p>`)
...
export async function sendPaymentReceipt(opts: {
  ...
  const html = layout(`
    ...
    <p style="margin:0 0 24px;font-size:15px;color:#64748b;line-height:1.7;text-align:right;">
      هلا <bdi>${name}</bdi>، تم استلام دفعتك بنجاح.
    </p>
    ...
    <p style="margin:0;font-size:14px;color:#64748b;line-height:1.6;text-align:left;direction:ltr;">
      Hi <bdi>${name}</bdi>, your payment of <strong>${amount} ${currency}</strong> for <strong>${planName}</strong> has been received.
    </p>
  `, `<p style="margin:0 0 16px;font-size:12px;color:#94a3b8;">هذا البريد إيصال رسمي لعملية الدفع · This is an official payment receipt.</p>`)
...
export async function sendSubscriptionCancelled(opts: {
  ...
  const html = layout(`
    ...
    <p style="margin:0 0 24px;font-size:15px;color:#64748b;line-height:1.7;text-align:right;">
      <bdi>${name}</bdi>، تأسفنا لرؤيتك تغادر. تم إلغاء اشتراك <strong>${planNameAr}</strong>. ستحتفظ بوصولك حتى <strong>${accessUntil}</strong>.
    </p>
    ...
    <p style="margin:0;font-size:14px;color:#64748b;line-height:1.6;text-align:left;direction:ltr;">
      Hi <bdi>${name}</bdi>, your <strong>${planName}</strong> subscription has been cancelled. You'll keep access until <strong>${accessUntil}</strong>. We'd love to have you back!
    </p>
  `)
...
export async function sendPaymentFailed(opts: {
  ...
  const html = layout(`
    <h1 style="margin:0 0 12px;font-size:22px;font-weight:800;color:#1e293b;text-align:right;">فشلت عملية الدفع ⚠️</h1>
    <p style="margin:0 0 24px;font-size:15px;color:#64748b;line-height:1.7;text-align:right;">
      هلا <bdi>${name}</bdi>، ما قدرنا نجدد اشتراك <strong>${planNameAr}</strong>. لو سمحت حدّث بيانات الدفع لتجنب انقطاع الخدمة.
    </p>
    ...
    <p style="margin:0;font-size:14px;color:#64748b;line-height:1.6;text-align:left;direction:ltr;">
      Hi <bdi>${name}</bdi>, we couldn't process your <strong>${planName}</strong> renewal. Please update your payment method to avoid losing access.
    </p>
  `)
